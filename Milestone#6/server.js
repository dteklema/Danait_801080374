...
const swaggerOptions = {
    swaggerDefinition: {
        info: {
            ...
            description: 'This is a documentation for users management',
            constact: {
                name: 'Danait Teklemariam',
                email: 'dteklema@uncc.edu'
            }
        },
        servers: ['http://167.172.150.145:5000']
    },
    apis: ['server.js']
}
const swaggerDocs = swaggerJSCodeModule(swaggerOptions);
expressHandler.use('/docs', swaggerUIExpressModule.serve, swaggerUIExpressModule.setup(swaggerDocs))
...
expressHandler.get('/users', function (requestObject, responseObject){
    dbHandler.all('SELECT * FROM users', (selectError, result)=>{
        ...
        responseObject.render("content".results)
    })
});
